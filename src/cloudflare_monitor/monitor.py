...
logging.basicConfig(
    level=LOG_LEVEL,
    format="%(asctime)s - %(levelname)s - (LocationMonitor) - %(message)s",
    stream=sys.stdout,
)
        ...
        try:
            with open(STATUS_FILE) as f:
                ...
                if not isinstance(data, dict):
                    logging.warning(
                        f"Status file contains invalid data format, expected dict got {type(data)}"
                    )
                    return {}
                return data
        ...
        if NOTIFY_ON_RESTART == "false":
            ...
            for diff in differences:
                ...
                if notifications.OPSGENIE_ENABLED:
                    ...
                    opsgenie_message = (
                        f"Cloudflare Status Sync: {diff['location']} is {label}"
                    )
            ...
            logging.warning(
                f"Invalid NOTIFY_ON_RESTART value: {NOTIFY_ON_RESTART}. Using silent sync."
            )
            for diff in differences:
                last_statuses[diff["location"].lower()] = diff["to_status"]
            save_last_statuses()
...
def main() -> None:
    ...
    logging.info(
        f"🚀 Starting Cloudflare Location Monitor for: {', '.join(TARGET_LOCATIONS)}"
    )
    if notifications.slack_client:
        logging.info(
            "slack: Location notifications enabled (broadcasting to all bot channels)"
        )
    if notifications.OPSGENIE_ENABLED:
        logging.info("opsgenie: Location notifications enabled.")
    ...
    while True:
        try:
            ...
            if not current_matches:
                logging.warning(
                    f"No matching components found for targets: {', '.join(TARGET_LOCATIONS)}. Check your TARGET_LOCATIONS and Cloudflare's component list."
                )
            ...
            for loc, data in current_matches.items():
                if data["status"] != last_statuses.get(loc):
                    ...
                    logging.info(
                        f"🔔 STATUS CHANGE: {loc.title()} → {label} (Component: {data['component_name']})"
                    )

                    # Send Slack Alert
                    slack_fields = [
                        {"title": "Location", "value": loc.title()},
                        {"title": "Component", "value": data["component_name"]},
                    ]
                    notifications.send_slack_alert(
                        title="Cloudflare Location Status Update",
                        fields=slack_fields,
                        status=data["status"],
                    )
                    ...
                    notifications.send_opsgenie_alert(
                        ...
                        description=opsgenie_desc,
                        status=data["status"],
                        tags=["cloudflare", "location-monitor", loc],
                    )
